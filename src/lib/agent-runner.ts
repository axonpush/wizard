...
6. **Integrate the SDK** into each relevant entry point, using the appropriate channel ID env var.
   ...
   Use ${channelIdExpr} to parse the channel ID.${opts.language === "typescript" ? "\n   Make sure process.env values are available (e.g. via dotenv or framework built-in env loading)." : "\n   Make sure os is imported if using os.environ."}${opts.environment ? `\n   When initializing the AxonPush client, pass \`environment="${opts.environment}"\` (Python) or \`environment: "${opts.environment}"\` (TypeScript) so every event is tagged with this deployment environment.` : ""}${opts.enableSentry ? `\n\n7. **Install Sentry SDK** pointing at AxonPush. Use the helper from the AxonPush SDK (${opts.language === "python" ? (opts.environment ? `\`from axonpush import install_sentry; install_sentry(environment="${opts.environment}")\`` : "`import os; from axonpush import install_sentry; install_sentry(environment=os.getenv(\"AXONPUSH_ENVIRONMENT\") or os.getenv(\"SENTRY_ENVIRONMENT\") or os.getenv(\"APP_ENV\") or os.getenv(\"ENV\") or \"development\")`") : (opts.environment ? `\`import { installSentry } from '@axonpush/sdk'; import * as Sentry from '@sentry/node'; installSentry(Sentry, { environment: '${opts.environment}' });\`` : "`import { installSentry } from '@axonpush/sdk'; import * as Sentry from '@sentry/node'; installSentry(Sentry, { environment: process.env.AXONPUSH_ENVIRONMENT ?? process.env.SENTRY_ENVIRONMENT ?? process.env.NODE_ENV ?? 'development' });`")}). This builds the Sentry DSN automatically from AXONPUSH_API_KEY + AXONPUSH_CHANNEL_ID + AXONPUSH_HOST.` : ""}
